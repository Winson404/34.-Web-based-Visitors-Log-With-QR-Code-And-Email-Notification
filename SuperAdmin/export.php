	    ...
	    $registration = mysqli_real_escape_string($conn, $_POST['registration']);
	     ...
	     $visitor = [
	        ...
	        if ($registration === 'Registrations Today') {
			        $sql = "SELECT * FROM visitors WHERE DAY(date_registered) = DAY(CURDATE()) AND MONTH(date_registered) = MONTH(CURDATE()) AND YEAR(date_registered) = YEAR(CURDATE()) ORDER BY lastname";
			} elseif ($registration === 'Registrations in the Last 7 Days') {
			    $sql = "SELECT * FROM visitors WHERE date_registered >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) AND YEAR(date_registered) = YEAR(CURDATE()) ORDER BY lastname";
			} elseif ($registration === 'Registrations in the Last 30 Days') {
			    $sql = "SELECT * FROM visitors WHERE date_registered >= DATE_SUB(CURDATE(), INTERVAL 30 DAY) AND YEAR(date_registered) = YEAR(CURDATE()) ORDER BY lastname";
			} else {
